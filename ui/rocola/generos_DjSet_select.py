import discord
from ui.rocola.allgenres_Select.view_genero_DjSet import GeneroDjSet
from utils.controlls_Json.lectura import cargar_links_DjSet

class MenuGenerosDjSet(discord.ui.Select):
    def __init__(self):
    ...
    async def callback(self, interaction:discord.Interaction):
        embed = discord.Embed(title="Modo DJ Set", color = discord.Color.red())
        embed.set_thumbnail(url="https://img.icons8.com/?size=100&id=VfM1DGzeu9I8&format=png&color=000000")
        embed.add_field(name="Selecciona el set de tu agrado", value="")
        selected_genero = self.values[0]
        await interaction.response.edit_message(embed=embed, view=GeneroDjSet(selected_genero), delete_after=180)
